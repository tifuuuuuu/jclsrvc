...
- **`jc` CLI** — [`jc.js`](jc.js) is a zero-dep client over the same REST API: `jc login <token>`, then `deploy`, `logs -f`, `exec`, `env`, `scale`, `stats`, `domains`, `roles`, `activity`, `backup`, and the lifecycle actions. Run `jc help` for the list. Per-app plan, health path, idle sleep, rate limit and cron jobs travel in the repo's `jerrick.json` rather than as CLI flags.
